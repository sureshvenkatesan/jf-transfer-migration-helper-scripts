...
if [ $# -ge 6 ]; then
    ...
    run_migrate_command "$src_list_in_dot_folder_command" "$target_list_in_dot_folder_command" "$root_folder" "0"
else
    root_folder=""
    # check for files in the root folder:
    src_command1="jf rt curl -s -XPOST -H 'Content-Type: text/plain' api/search/aql --server-id $source_artifactory --insecure \
    ...
    #Call the migrate command without the trailing * in $folder_to_migrate
    run_migrate_command "$src_list_in_dot_folder_command" "$target_list_in_dot_folder_command" "." "0"
...
#    echo $target_list_command
    ...
    run_migrate_command "$src_list_in_this_folder_command" "$target_list_in_this_folder_command" "$folder_to_migrate" "$((folder_position + 1))"
   ...
   #Call the migrate command with the trailing * in $folder_to_migrate
    #folder_to_migrate="${folder_to_migrate/%\*/}"  # Remove the trailing "*"
    run_migrate_command "$src_list_in_subfolders_command" "$target_list_in_subfolders_command" "$folder_to_migrate" "$((folder_position + 1))"
